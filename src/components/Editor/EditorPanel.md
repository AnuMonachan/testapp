...
User interface for editing each section of a singular story
...
![OBS Image](https://cdn.door43.org/obs/jpg/360p(x/obs-en-01-15.jpg)
...
function EditorPanelExample() {
  ...
  useEffect(() => {
    actions.setStory(mdToJson(mdString));
  }, []);

  return (
    <>
      <EditorPanel obsStory={state.story} updateSection={actions.updateSection} />
    </>
  )
  }
